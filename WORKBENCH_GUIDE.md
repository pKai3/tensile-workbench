...
In the **Calculation inspector**, the selected specimen's check messages appear immediately above the graph. They refresh during unsaved override previews and are hidden when nothing is flagged. Expand **Inspector controls and calculation details** below the chart to access the individually collapsible display-item controls, fit/EL editors, property comparisons, endpoint details, gauge model, source information and method explanation. The specimen selector and view buttons stay outside this fold.
