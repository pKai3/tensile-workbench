...
Detection uses original acquisition order, preferably force versus time, with engineering stress as an explicitly recorded force proxy when no force channel exists. Missing/non-increasing time uses original row spacing. Repeated or reversing strain does not erase the fracture drop. Windows of several sizes look for a substantial load loss (at least 5% of peak, and above the estimated noise floor) that is faster than preceding necking and does not recover. The dominant qualifying collapse is selected rather than the first steepening. A local rate/noise threshold then works backwards within that rapid drop to the last recorded point before it. There is no fixed strain setback or strain-grid interpolation. Gaps in load readings or large time gaps cannot form the detected drop.

The minimum loss, sixfold decline contrast and local onset threshold are heuristic defaults, not validated instrument settings. The old saved `fracture_slope_fraction` setting is retained for compatibility but is no longer used. Audit records the signal, time/row basis, selected original row and timestamp, first collapse row, load-loss evidence, detection thresholds and status. If recording ends during the drop, limited follow-through is noted. A recorded machine unloading or an artefact can still resemble fracture: review the purple marker, especially for unusual or multi-stage failures. Detection is independent of Instron EL and performed before reconstruction; transformed curves reuse that same physical endpoint. Curve integration excludes rows acquired after it, including post-fracture strain reversals.
