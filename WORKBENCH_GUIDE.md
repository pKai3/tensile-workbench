...
- **Specimens:** **Data use** dropdowns at the far left and an **Applies to** selector, followed by the same properties and source-specific EL columns as Summary. Calc elongation always shows the unreconstructed detected endpoint; Reconstruct shows its correction separately. Calc toughness uses the group's selected measured/reconstructed basis. Instron remains the original reported value where provided. Peak-force row numbers, timestamps, final-reading bookkeeping and plotting-grid diagnostics remain in the inspector/audit data. Useful fit, geometry and **EL source for plots** fields remain at the right. Operator **Specimen text input** is displayed when available; the underlying CSV identity remains in the tooltip and export. A single **Checks** column contains only failures/review items, one per line; it is blank when nothing is flagged. Excluded rows remain dimmed; individually excluded properties remain visible but crossed out.
...
specimen table · inclusion, checks and data sources** section. Filters, Data use
controls and the specimen-specific Checks column remain visible outside it.

The **Data use** dropdown is **global by default**, saved in the personal project file. **Exclude specimen entirely** applies to existing and future graphs, including statistics, Instron comparisons, both tensile averages, both work-hardening methods, representative selection and individual plots/exports.

For an exception, first change **Applies to** to **This graph only**, then set Data use. This explicitly overrides the global setting for this specimen in this graph. Choosing **Global default** again removes the exception and restores the current global choice; it does not copy the local choice into the global setting. Changes to the global default leave other graphs' explicit exceptions untouched. The row shows the global data-use mode while an override is active.
...
Fully excluded specimens remain in per-specimen/diagnostic exports, with effective inclusion, data-use mode, excluded properties, scope, global default, specimen identity and reason; they never contribute to averages. Searching/sorting is display-only. With live preview off, changing Data use clears the old plots and requires **Update plots**. With live preview on, plots update automatically. Tables update independently of plot selection. All selection settings are private/ignored, not shared deployment defaults.

Partial exclusions distinguish **AVE failed after UTS / Broke outside dots** (retain YS, UTS, E, uniform EL and pre-peak WH), **AVE failed after yield, before UTS** (retain YS, UTS and E only), and **AVE unreliable throughout** (retain UTS only). The outside-dots option assumes strain remained reliable through UTS. Stress must remain trustworthy; YS requires trustworthy strain through the offset intersection. All partial modes omit failure EL, reconstructed EL, full-test toughness and full tensile shape. The same eligibility applies to imported Instron properties. Raw values remain available for inspection and audit rather than being deleted.

**Group curve review** overlays one selected group's raw curves without fracture trimming, reconstruction or smoothing. It loads on opening the tab. Specimens are sorted naturally by displayed name (1, 2, 10). Click a curve or specimen name to highlight it; use **Inspect highlighted specimen** for fit/endpoint review. Each partial-exclusion mode has its own line style and a short label in the specimen key outside the plotting area. Hover shows the full mode. The key has no inclusion or visibility checkboxes: clicking a name only highlights the curve, even if clicked again. Data-use and scope controls in the table below are the only exclusion controls. Fully excluded specimens are visible by default as faint long-dashed lines; **Show fully excluded specimens** is a display-only filter. A recorded-time axis is available when strain itself is unreliable.

The review's **YS**, **UTS** and **EL** toggles independently show calculated landmarks on each measured curve, including saved manual fit/EL overrides. They default to off and retain their setting while switching groups or axes. These are not Instron or reconstructed markers. Excluded properties remain available diagnostically as hollow markers with an explicit exclusion label on hover; they still do not contribute to analysis. Missing values or coordinates produce no marker. The fully-excluded display filter applies to markers as well as curves. Toggling markers keeps the current zoom and never changes analysis settings.

Group-review lines use vector strokes with fixed pixel dash lengths, matching the legend swatches even when a specimen is highlighted. All measured points, missing-reading gaps and strain reversals remain in the plot data. Only the drawn path is simplified at the current zoom to keep dense curves responsive; no measurements or calculations are downsampled or smoothed.

**Landmark averaging with mixed eligibility:** trusted full curves supply the three stage shapes; each stage's mean shape is remapped between the eligible group mean landmark coordinates. YS/UTS can use more specimens than failure EL. Each coordinate has its own valid n and sample SD, retained in the curve export. Landmark WH uses eligible pre-peak shapes (including after-UTS AVE failures). Pointwise averages and representative curves use only full eligible curves. Without a trusted shape or ordered mean landmarks, no landmark curve is fabricated. Strength–EL scatter uses paired eligible specimens only; an on-screen warning lists partial exclusions and explains why scatter means can differ from the summary. Publication plots do not receive these warnings.
