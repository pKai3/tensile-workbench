...
Uniform elongation is engineering strain at the first maximum engineering stress. Analysis EL uses an automatically selected CSV endpoint: before a distinct abrupt load collapse, or a supported terminal estimate after progressive load loss. Toughness is the engineering stress–strain integral through that measurement. Gauge reconstruction uses the same endpoint. A terminal estimate remains explicitly review-flagged; it is not a confirmed separation point. No calculation is replaced by an Instron summary value.

EL provenance is explicit:

- **Instron:** the imported Strain 1 at break result.
- **Calc:** strain at the selected CSV endpoint, before any landmark shape setback. Audit distinguishes abrupt-collapse detection from a terminal estimate.
...
Detection uses original acquisition order, preferably force versus time, with engineering stress as an explicit force proxy when no force channel exists. Missing/non-increasing time uses original row spacing. Multi-size windows seek a substantial non-recovering loss (at least 5% of peak and above noise). A local abruptness check compares unloading rates with the preceding eight increments and requires substantial loss within the next eight increments. A small spike embedded in broad necking therefore does not qualify solely by contrast with an earlier plateau. A local rate/noise threshold then selects the last recorded point before the rapid drop. There is no strain setback or interpolation. Load/time gaps cannot form the detected drop.

If no distinct abrupt event is resolved, the algorithm can use a **terminal estimate** without a ductile/brittle selector. This requires load to have fallen to at most 50% of peak, ongoing decline above noise, no load/time gap since peak, no sustained recovery, and a usable terminal strain beyond peak-load strain. It uses the final recorded measurement, or the last load-bearing measurement before a sustained near-zero-load tail. The near-zero floor is the greater of 0.1% of peak and 12 times the estimated noise. It does not search for the maximum strain or assume every export ends at fracture. **Estimated** status and an EL review flag identify these cases because termination during load loss cannot establish physical separation.

These thresholds, the sixfold decline contrast and the eight-increment local window are heuristic defaults, not validated instrument settings; results can depend on acquisition rate. Audit retains the endpoint kind, review reason, selected row/time, signal, load-loss evidence and thresholds. Machine unloading can resemble specimen failure. Selection is independent of Instron EL and performed before reconstruction; all downstream calculations reuse that selected measurement. The legacy `fracture_slope_fraction` setting is retained but unused.

In the landmark calculation, the pre-break **shape** endpoint is normally the selected endpoint minus 0.05 percentage points; its post-UTS shape is mapped back to the selected EL landmark. The setback does not reduce reported EL. A supported terminal estimate is available to plots, reconstruction and toughness, while remaining flagged for review. With neither endpoint supported, **Not detected** leaves EL/toughness and reconstruction unavailable. Specimens are not automatically excluded. YS, UTS, uniform elongation and both WH methods remain available when their own pre-peak requirements are satisfied; landmark WH needs no fracture endpoint.
...
proxy. The endpoint is either a detected abrupt-collapse onset or a review-flagged
terminal estimate, never the deliberately earlier landmark shape-trimming point.
...
Select **0.2% YS vs elongation** and/or **UTS vs elongation** under Plot views for any graph definition. Both use the current analysis EL on the x-axis and strength in MPa on the y-axis. EL uses the selected CSV endpoint (including review-flagged terminal estimates), or its gauge reconstruction—not Instron's break result, uniform elongation or a post-fracture gauge-length measurement.
