...
def export_results(frames, path, metadata):
    ...
    metadata = {**metadata, 'definitions': {
        'calculated_basis': 'Tensile plots/statistics use the selected CSV endpoint, or its gauge reconstruction. A valid saved manual row overrides automatic selection. Toughness integrates through the chosen measurement. Without an override, a distinct abrupt collapse uses its pre-collapse reading; otherwise supported progressive unloading can use a review-flagged terminal estimate. This does not establish physical separation. Pre-peak properties and WH use measured data. No Instron substitution.',
        'elongation_columns': 'Instron is imported Strain 1 at break. Calc is the selected CSV endpoint (Detected abrupt collapse or Estimated terminal load loss), without a strain setback. Reconstruct applies the gauge model to that same endpoint and is blank unless enabled and available. Audit preserves endpoint kind, review reason, original rows/time, load-loss evidence and settings, plus last valid/max retained strain.',
        ...
        'failure_endpoint': 'Acquisition-order endpoint selection (v3). Abrupt collapse requires sustained loss plus local abruptness. Without a resolved abrupt break, terminal estimates require continuing loss to at most 50% of peak, no sustained recovery or post-peak acquisition gaps, and usable terminal strain. Sustained near-zero-load tails are excluded when present. Terminal estimates are always review-flagged, not proof of separation. Heuristic thresholds and acquisition-rate dependence are documented in the guide; no Instron EL or strain-grid interpolation is used.',
